...
object TreeTransforms {
  ...
  abstract class TreeTransformer extends Phase {
    ...
    def mutateTransformers[T](info: TransformerInfo, mutator: Mutator[T], mutationPlan: Array[Int], tree: T, cur: Int)(implicit ctx: Context) = {
      ...
      var allDone = i < l
      while (i < l) {
        // @dDarkDimius: Please don't use T,L as parts of names. Nobody can see what
        // it means. Generally, we need to pay a lot of attention to consistency of naming
        // If something is named one way in typers, use the same scheme, don't reinvent a new one.
        // E.g. if it's typedNamed and typedUnnamed,
        // then don't use transformNameTree and transformUnnnamed - you
        // have just introduced a double inconsistency!
