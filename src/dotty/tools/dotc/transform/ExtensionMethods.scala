...
class ExtensionMethods extends MacroTransform with IdentityDenotTransformer { thisTransformer =>
  ...
  def newTransformer(implicit ctx: Context): Transformer =
    new Extender
  ...
  class Extender extends Transformer {
    ...
    override def transformStats(stats: List[Tree], exprOwner: Symbol)(implicit ctx: Context): List[Tree] =
      super.transformStats(stats, exprOwner) map {
        case impl @ Template(constr, parents, selfType, body) =>
          extensionDefs remove impl.symbol.owner match {
            case Some(defns) if defns.nonEmpty =>
              cpy.Template(impl, constr, parents, selfType, body ++ defns)
            case _ =>
              impl
          }
        case stat =>
