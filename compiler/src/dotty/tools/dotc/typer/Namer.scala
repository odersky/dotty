...
class Namer { typer: Typer =>
  ...
  def valOrDefDefSig(mdef: ValOrDefDef, sym: Symbol, typeParams: List[Symbol], paramss: List[List[Symbol]], paramFn: Type => Type)(implicit ctx: Context): Type = {
    ...
    def inferredType = {
      ...
      // Widen rhs type and eliminate `|' but don't widen:
      //  - Module singleton types,
      //  - Singleton types if definition is transparent,
      //  - ConstantTypes if definition is final.
      def widenRhs(tp: Type): Type = tp.widenTermRefExpr match {
        case ref: TypeRef if ref.symbol.is(ModuleClass) => tp
        case stp: SingletonType if sym.is(Transparent, butNot = Method | Mutable) => stp
        case ctp: ConstantType if sym.is(Final, butNot = Method | Mutable) => ctp
